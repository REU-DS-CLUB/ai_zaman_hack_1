...
## 1. Задача, под которую заточен `h2.py`
...
`h2.py` специально спроектирован так, чтобы на этих метриках показать устойчиво высокий результат без использования больших языковых моделей.
...
`h2.py` опирается на несколько словарных ресурсов. Их можно обновлять и расширять — это основная точка кастомизации.
...
- `h2.py` берёт из него только те слова, которые содержат подстроку из `toxic_substrings.json`;
...
Внутри `h2.py` основная функция — `detox(text: str) -> str`.
...
```bash
python h2.py -i dev_inputs.tsv -o submit_h2.tsv
```
...
- можно добавлять небольшой слой поверх `h2.py`, который:
...
Сам `h2.py` при этом остаётся стабильным ядром, а модель — лишь опциональным постпроцессингом.
